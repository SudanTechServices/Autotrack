 ...
 */
@WebServlet("/UDR_Location_Excel")
public class UDR_Location_Excel extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UDR_Location_Excel() {
        super();
		...
		OutputStream out = null;
		  try {
			  response.setContentType("application/vnd.ms-excel");
			   response.setHeader("Content-Disposition", "attachment; filename=location_grid.xls");
			  ...
			  // Class.forName("org.postgresql.Driver").newInstance();
			   ...
			   HSSFSheet sheet = wb.createSheet("Location Report Sheet");
			  ...
			  /* sheet.addMergedRegion(new Region(0,(short)0,0,(short)3));
			   ...
			   HSSFCell cell2B = rowhead.createCell(0);  
		        cell2B.setCellValue(new HSSFRichTextString("Report for Location " +locations));  
			  ...
			  /* HSSFRow rowhead2 = sheet.createRow((short)2);
			   ...
			   if (select != null && select.length != 0) {
				   ...
				   rs = st.executeQuery("SELECT * FROM student WHERE route_number='"+route+"' order by enrolment_number");	for (int i = 0; i < select.length; i++) {
