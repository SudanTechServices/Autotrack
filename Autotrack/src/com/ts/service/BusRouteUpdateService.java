...
import com.ts.dao.RouteUpdateDAO;
...
public class BusRouteUpdateService extends CommonService{
int counter;
	@Override
	public void run() throws ServletException, IOException {
	
				BusRouteInfo busrouteinfoobj =new BusRouteInfo();
				RouteUpdateDAO busrouteinfodao=new RouteUpdateDAO();
				
				ses=request.getSession();
				busrouteinfoobj.setBus_route_number(request.getParameter("route_number"));
				//System.out.println(request.getParameter("route_number"));
			   ...
			   int row=busrouteinfodao.busrouteUpdateValidate(busrouteinfoobj);
				...
				if(row>0)
						{
							RequestDispatcher rd=request.getRequestDispatcher("/JSP/routeregistration.jsp?message=updated");
